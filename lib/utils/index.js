...
/**
 * 将站内 slug/path 转为绝对 URL；外部 URL 返回 null，避免 sitemap 出现错误拼接。
 * @param {string} siteUrl
 * @param {string} path
 * @returns {string|null}
 */
export function createSiteUrl(siteUrl, path = '') {
  ...
  if (
    ...
    isHttpLink(normalizedPath)
  ) {
    return null
  }

  return `${normalizedSiteUrl}/${normalizedPath.replace(/^\/+/, '')}`
}
