...
import { findPostByRoute } from '../utils/postRoute'
...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  const findPost = () => {
    ...
    return findPostByRoute(props.allPages, fullSlug, lastSegment)
  }
...
function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  ...
  const description = collection?.description
    ...
    : defaultDescription
  const blockPageCover = block?.format?.page_cover
  const pageCover = collection?.cover
    ? mapImgUrl(collection?.cover, collection, 'collection')
    : blockPageCover
      ? mapImgUrl(blockPageCover, block)
      : defaultPageCover
  ...
  const notionInternalIcon = typeof icon === 'string' && icon.startsWith('notion://')
  if (!icon || emojiPattern.test(icon) || notionInternalIcon) icon = defaultIcon
